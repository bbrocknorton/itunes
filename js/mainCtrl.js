...
app.controller('mainCtrl', function($scope, itunesService) {
  ...
  //the 'data' property. The value is 'songData'. That means ng-grid is looking for songData on $scope and is putting whatever 
  //songData is into the grid.  This means when you make your iTunes request, you'll need to get back the information, parse it 
  ...
  //Our controller is what's going to connect our 'heavy lifting' itunesService with our view (index.html) so our user can see 
  //the results they get back from itunes.
  ...
  //whenever the user clicks the submit button //*remember, that method should be expecting an artist name. The artist name is 
  //coming from the input box on index.html, head over there and check if that input box is tied to any specific model we could use.
    ...
    itunesService.getSongs($scope.artist).then(function(songs) {
      ...
      $scope.songData = songs.map(function(itunesFormattedResult) {
        return {
          ...
          Collection: itunesFormattedResult.collectionCensoredName,
      ...
      //do stuff with the stuff that came back from itunes
    })
  }
  ...
  //If everything worked you should see a huge array of objects inside your console. That's great! But unfortunately that's not what
  // ng-grid is expecting. What you need to do now is sort the data you got back to be an object in the following format.
  /*
